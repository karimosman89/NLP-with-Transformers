...
class DataImporter:
    ...
    def load_data(self):
        data_path = os.path.join("jun24_bmlops_classification_e-commerce/mlflow/data/preprocessed/X_train_update.csv")
        data = pd.read_csv(data_path)
        data["description"] = data["designation"] + str(data["description"])
        data = data.drop(["Unnamed: 0", "designation"], axis=1)

        target_path = os.path.join("jun24_bmlops_classification_e-commerce/mlflow/data/preprocessed/Y_train_CVw08PX.csv")
        ...
        mapper_path = os.path.join("jun24_bmlops_classification_e-commerce/mlflow/models/mapper.pkl")
        with open(mapper_path, "wb") as fichier:
...
class ImagePreprocessor:
    def __init__(self, base_path, image_subpath="jun24_bmlops_classification_e-commerce/mlflow/data/raw/image_train"):
        self.image_path = os.path.join(base_path, image_subpath)

    def preprocess_images_in_df(self, df):
        df["image_path"] = (
            self.image_path + "/image_"
