...
class TextLSTMModel:
    ...
    def preprocess_and_fit(self, X_train, y_train, X_val, y_val):
        ...
        with open("dags/models/tokenizer_config.json", "w", encoding="utf-8") as json_file:
        ...
        lstm_callbacks = [
            ModelCheckpoint(
                filepath="dags/models/best_lstm_model.keras", save_best_only=True
            ),
            EarlyStopping(
    ...
    def save(self, filepath):
        if self.model:
            ...
            raise ValueError("Model is not built yet. Please train the model before saving it.")

    def save_weights(self, filepath):
        if self.model:
            self.model.save_weights(filepath)
        else:
            raise ValueError("Model is not built yet. Please train the model before saving weights.")
...
class ImageVGG16Model:
    ...
    def preprocess_and_fit(self, X_train, y_train, X_val, y_val):
        ...
        vgg_callbacks = [
            ModelCheckpoint(
                filepath="dags/models/best_vgg16_model.keras", save_best_only=True
            ),
            EarlyStopping(
                patience=3, restore_best_weights=True
            ),
