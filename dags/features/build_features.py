...
class DataImporter:
    ...
    def load_data(self):
        data_path = os.path.join(self.base_path, "data/preprocessed/X_train_update.csv")
        data = pd.read_csv(data_path)
        data["description"] = data["designation"] + str(data["description"])
        data = data.drop(["Unnamed: 0", "designation"], axis=1)

        target_path = os.path.join(self.base_path, "data/preprocessed/Y_train_CVw08PX.csv")
        ...
        mapper_path = os.path.join(self.base_path, "models/mapper.pkl")
...
class ImagePreprocessor:
    def __init__(self, base_path, image_subpath="data/raw/image_train"):
